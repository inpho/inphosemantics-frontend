...
inpho.semantics.cache = {"corpora":{},"models":{}};
...
// corpora
inpho.semantics.getCorpusBackgroundColor = function(corpusKey){
  return inpho.semantics.cache["corpora"][corpusKey]["backgroundcolor"];
}

inpho.semantics.getCorpusFontColor = function(corpusKey){
  return inpho.semantics.cache["corpora"][corpusKey]["fontcolor"];
}

inpho.semantics.getCorpusLongLabel = function(corpusKey){
  return inpho.semantics.cache["corpora"][corpusKey]["long label"];
}

// models
inpho.semantics.getModelBackgroundColor = function(modelKey){
  return inpho.semantics.cache["models"][modelKey]["backgroundcolor"];
}

inpho.semantics.getModelFontColor = function(modelKey){
  return inpho.semantics.cache["models"][modelKey]["fontcolor"];
}

inpho.semantics.getModelLongLabel = function(modelKey){
  return inpho.semantics.cache["models"][modelKey]["long label"];
}
...
inpho.semantics.populate_selection_modal = function (elt) {
  ...
  for (corpus in inpho.semantics.cache["corpora"]) {
    var corpusDescription = inpho.semantics.getCorpusLongLabel(corpus);
  ...
  for (model in inpho.semantics.cache["models"]) {
    var modelDescription = inpho.semantics.getModelLongLabel(model);
  ...
  if (params.corpora) {
    for (var ci = 0; ci < params.corpora.length; ci++) {
      ...
      var label = inpho.semantics.getCorpusLongLabel(selectedCorpus);
  ...
  if (params.models) {
    for (var mi = 0; mi < params.models.length; mi++) {
      ...
      var label = inpho.semantics.getModelLongLabel(selectedModel);
...
inpho.semantics.exportQuery = function(corpus, corpusParam, model, phrase, width){
...
$("exportExportBtn").click( function(event){
  ...
  var param  = $("#exportCorpus").val();
  var model  = $("#exportModel").val();
  var phrase = $("#exportPhrase").text();
  var width  = $("#exportCount").text();
  inpho.semantics.exportQuery(corpus, param, model, phrase, width);
});
...
inpho.semantics.build_table = function (elt, query) {
  ...
  $.get("/data", query, 'json')
    .success( function(data) {
      ...
      corpusDiv.innerHTML = inpho.semantics.getCorpusLongLabel( query.corpus );
      corpusDiv.style.backgroundColor = inpho.semantics.getCorpusBackgroundColor( query.corpus );
      corpusDiv.style.color = inpho.semantics.getCorpusFontColor( query.corpus );
      $(div).append( corpusDiv );

      var modelDiv = document.createElement('div');
      modelDiv.className = "queryHeader";
      modelDiv.innerHTML = inpho.semantics.getModelLongLabel( query.model );
      modelDiv.style.backgroundColor = inpho.semantics.getModelBackgroundColor( query.model );
      modelDiv.style.color = inpho.semantics.getModelFontColor( query.model );
      $(div).append( modelDiv );
      
      var phraseDiv = document.createElement('div');
      phraseDiv.className = "queryHeader"; // currently white background, so no reason to make the font white.
  ...
  for(var mi=0; mi < params.models.length; mi++) {
    for(var ci=0; ci < params.corpora.length; ci++) {
      for(var pi=0; pi < params.phrases.length; pi++) {
        if (!(params.corpora[ci] && params.models[mi] && params.phrases[pi]))
          continue;
