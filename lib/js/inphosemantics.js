...
inpho.semantics.exportQuery = function(corpus, model, phrase, width){
  ...
  $.get("/export.csv", query, 'json')
    .success( function(data) {
      ...
      $(".modal-footer", "#exportModal").append("<a style='float: left' href=" + url + ">matrix</a>");
    })
    .error( function(error) {
      ...
      var errordiv = "<div class='alert alert-error'>" + error.responseText + "</div>";
...
$("#exportExportBtn").click( function(event){
  ...
  // Remove ...
  $("a", "#exportModal").remove(); // old results
