function main() {
  ...
  $.getJSON(jsondir, function(json) {
    ...
    function getCorpusLongLabel(corpusKey) {
      ...
      console.log('Corpus key not found.')
    }
    ...
    function getModelLongLabel(modelKey) {
      ...
      console.log('Model key not found.')
    }
    ...
    $("#searchLimit").submit(function(event){
      event.preventDefault();
      setSearchLimit( $("#limit").val() );
    });
    ...
    var hashSearchLimit = obj['searchLimit'];
    if (!hashSearchLimit) {
      hashSearchLimit = 20;
    }
    $("#limit").val(hashSearchLimit);
    ...
    function renderTable(corpora, models, phrases) {
      ...
      $("#root > tbody > tr > td > table > tbody > tr > td > table > tbody > tr").each(function() {
        for (var i=0; i<phrases.length; i++) {
          ...
          var classes ='table phrase';

          // determine if background coloured
          if (count++ % 2 == 0){
            classes += " odd-nth-phrase";
          }

          $(this).append(
            "<td><table class='"+classes+"' id='"+$(this).closest("table").attr("id")+":"+phrase+"'><thead><tr><th colspan='2' style='text-align: center;'>"+phrase+"</th></tr></thead><tbody></tbody></table></td>"
          );
        }
      });
    }
    ...
    function renderData(){
      ...
      var searchLimit = hashObj['searchLimit'];
    ...
    $("#queryReset").click(function(){
      // clean out each currently selected corpora / models / phrases.
      var hashObj = convertHashToObject(window.location.hash);

      var corpora = hashObj['corpora'];
      for (var i = 0; i < corpora.length; i++){
        var li = document.getElementById(corpora[i] + "li");
        $(li).remove()
      }

      var models = hashObj['models'];
      for (var i = 0; i < models.length; i++){
        var li = document.getElementById(models[i] + "li");
        $(li).remove()
      }

      var phrases = hashObj['phrases'];
      for (var i = 0; i < phrases.length; i++){
        var li = document.getElementById(phrases[i] + "li");
        $(li).remove()
      }
      window.location.hash = "";
      window.location.reload(); // kind of don't like this line.
      // because "reset" is not the same thing as clear.
