...
$(document).ready( function() {
  ...
  $.getJSON(jsondir, function (json) {
    ...
    var modelsSet = {};
    // cache each corpus
    for (corpusShortLabel in json) {
      var corpusLongLabel = json[corpusShortLabel]["long label"];
      inpho.semantics.cache["corpora"][corpusShortLabel] = corpusLongLabel;

      var corpus = json[corpusShortLabel];
    ...
    // cache each model
    for(modelShortLabel in modelsSet) {
      var modelLongLabel = modelsSet[modelShortLabel]["long label"];
      inpho.semantics.cache["models"][modelShortLabel] = modelLongLabel;
    }
...
// *** CACHE INTERACTION *** //
inpho.semantics.getCorpusLongLabel = function(corpusShortLabel){
  return inpho.semantics.cache["corpora"][corpusShortLabel];
}

inpho.semantics.getModelLongLabel = function(modelShortLabel){
  return inpho.semantics.cache["models"][modelShortLabel];
}
...
inpho.semantics.build_table = function (elt, query) {
  ...
  $.get("/data", query, 'json')
    .success( function(data) {
      ...
      $(div).append("<div>" + inpho.semantics.getCorpusLongLabel( query.corpus ) + "</div>");
      $(div).append("<div>" + inpho.semantics.getModelLongLabel( query.model )  + "</div>");
