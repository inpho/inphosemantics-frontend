...
/**
   ...
   the data.
*/
inpho.semantics.build_table = function (elt, query, data) {
  ...
  for(var ci=0; ci < params.corpora.length; ci++) { // For each corpus,
    for(var mi=0; mi < params.models.length; mi++) { // for each model,
      for(var pi=0; pi < params.phrases.length; pi++) { // and for each phrase...
        ...
        inpho.semantics.cache["phrases"][phrase] = {"backgroundcolor":color};

        
        // ... and send it.
        $.get("/data", query, 'json')
          .success( function(data) {
            return inpho.semantics.build_table(elt, query, data);
          })
