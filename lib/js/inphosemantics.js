...
inpho.semantics.program_button = function(select, button, list) {
  $(button).click( function(event) {
    ...
    var anchor = "<a class='close remove'>" + " x " + "</a>";
    var li = "<li>" + anchor + $("option:selected",select).text() + "</li>";
    $(list).append( li );
  });
}

inpho.semantics.program_button( $('#corporaSelect'), $('#corporaSubmit'), $('#currentCorpora') );
inpho.semantics.program_button( $('#modelsSelect'),  $('#modelsSubmit'),  $('#currentModels')  );

$("#search").submit( function(event) {
  event.preventDefault();
  var anchor = "<a class='close remove'>" + " x " + "</a>";
  var li = "<li>" + anchor + $("#searchTerm").val()  + "</li>";
  $("#currentPhrases").append( li );
  this.reset();
});
